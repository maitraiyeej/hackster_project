...
const UserProfile = () => {
    ...
    if (loading) {
    return (
        <LoadingScreen message='FETCHING_USER PROFILE...'/>
    );
}
            ...
            <div className="flex justify-between items-center mb-8">
                ...
                {isOwner && !isEditing && (
                    <button 
                        ...
                        className="border-2 bg-black text-white px-6 py-2 text-[10px] font-bold uppercase tracking-widest hover:invert transition-all"
                    >
                        Edit Profile
                    </button>
                )}
                        ...
                        <div className="flex justify-between items-start mb-10">
                            ...
                            {profile.githubUrl && (
                                <a 
                                    ...
                                    target="_blank" 
                                    rel="noreferrer" 
                                    className="group border-4 border-black p-3 bg-white hover:bg-black transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px]"
                                >
                                    <svg 
                                        role="img" 
                                        viewBox="0 0 24 24" 
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {profile.skills?.length > 0 ? (
                                        profile.skills.map((skill, index) => (
                                            <span key={index} className="text-[10px] font-bold border border-black px-2 py-1 uppercase">
                                                {skill}
                                            </span>
                                        ))
                                    ) : (
